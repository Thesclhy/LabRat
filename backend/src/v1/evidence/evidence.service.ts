...
import { experimentLabelFromWorkbookName } from "../../saas/regionExtractionTemplates.js";
import { resolveExperimentLink } from "../../saas/regionTemplateApplications.js";
...
const resolveExperimentLinkCompat = resolveExperimentLink as LegacyDomainOperation;
...
export class EvidenceService {
  ...
  // creating a template or a version: a block drawn by hand in a file the
  // template could not match still counts as the same kind of data.
  async linkWorkbookReviewRegion(
    auth: AuthContext,
    sessionId: string,
    regionId: string,
    input: LinkWorkbookReviewRegionDto,
  ) {
    const { region } = await this.region(auth, sessionId, regionId, "approve");
    const dataKind = String(input.dataKind || "").trim();
    if (!dataKind) throw new ApiError(400, "data_kind_required", "Provide the data kind to link this region as.");
    if (!region.acceptedRevisionId) {
      throw new ApiError(409, "region_not_confirmed", "Confirm the region before linking it to a data kind.");
    }
    const identities = await this.repository.listExperimentIdentities(String(region.projectId));
    const requestedExperimentId = String(input.linkedExperimentId || "").trim();
    let experimentLabel = String(input.experimentLabel || "").trim() || null;
    let link: Record<string, any>;
    if (requestedExperimentId) {
      const identity = identities.find((item) => item.id === requestedExperimentId);
      if (!identity) throw new ApiError(404, "experiment_identity_not_found", "The chosen experiment does not belong to this project.");
      experimentLabel = experimentLabel || identity.canonicalLabel || null;
      link = { linkedExperimentId: identity.id, linkStatus: "resolved", candidates: [{ experimentId: identity.id, label: identity.canonicalLabel || identity.id }] };
    } else {
      if (!experimentLabel) {
        const sourceDocument = region.sourceDocumentId ? await this.repository.findSourceDocumentById(String(region.sourceDocumentId)) : null;
        const metadata = (sourceDocument?.metadata || {}) as Record<string, unknown>;
        experimentLabel = experimentLabelFromWorkbookName(String(metadata.workbookName || metadata.fileName || ""));
      }
      link = resolveExperimentLinkCompat({ identities, experimentLabel });
    }
    const linkedExperimentId = link.linkedExperimentId || region.linkedExperimentId || null;
    const updated = await this.repository.updateWorkbookReviewRegion(String(region.id), {
      dataKind,
      linkedExperimentId,
      expectedVersion: region.version,
      updatedBy: auth.user.id,
    });
    await this.auditRegion(auth, region, "link", "workbook_review_region", String(region.id),
      `Linked workbook review region ${region.sheetName}!${region.rangeRef} as ${dataKind}${linkedExperimentId ? ` for ${experimentLabel || linkedExperimentId}` : ""}.`,
      { dataKind, linkedExperimentId, linkStatus: link.linkStatus });
    return {
      region: await workbookReviewRegionSummary(this.repository, updated || region),
      link: {
        regionId: region.id,
        dataKind,
        linkedExperimentId,
        experimentLabel,
        linkStatus: linkedExperimentId ? (link.linkStatus === "resolved" ? "resolved" : "already_linked") : link.linkStatus,
