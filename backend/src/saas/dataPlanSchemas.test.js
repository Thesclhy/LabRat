...
function experimentRecordPlan(overrides = {}) {
  return {
    ...
    dependencyHashes: [
      { kind: "workbook_understanding", id: "wu_1", hash: "sha256_understanding" },
    ...
    operations: [
      { op: "read_table_region", evidenceKey: "wu_1:fact_1", sourceDocumentId: "source_doc_1", sheetName: "Runs", range: "A1:C3" },
      { op: "use_row_as_header", evidenceKey: "wu_1:fact_1", rowNumber: 1 },
      { op: "bind_experiment_identity", evidenceKey: "wu_1:fact_1", experimentAxis: "rows", column: "A" },
      { op: "bind_fields", evidenceKey: "wu_1:fact_1", fields: [{ column: "B", fieldKey: "temperature", role: "condition", valueType: "number", unit: "degC" }] },
      { op: "select_data_rows", evidenceKey: "wu_1:fact_1", startRow: 2, endRow: 3, skippedRows: [] },
      { op: "emit_experiment_records", evidenceKey: "wu_1:fact_1" },
    ],
    ...overrides,
  };
}
