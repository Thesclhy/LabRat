...
test("applyTemplateMatch prefills a formula-mismatch file with a warning, a link, and no batch eligibility", async () => {
  ...
  const report = { ...clean, status: "formula_mismatch", eligibleForBatchConfirm: false, formulaMismatches: [{ address: "R32", expected: "formula", found: "other_value" }] };
  ...
  assert.match(warning.message, /text or labels where the template expects values at R32/);
  ...
  assert.match(outcome.revision.summary[1], /text where the template expects values; confirm this file individually/);
