...
const EXPERIMENT_BROWSER_PLAN_SYSTEM = [
  ...
  "Return exactly {requestSummary, sourceSelections, experimentSelections, fieldTargets, reviewPlan, displayPlan, warnings}.",
  ...
  "experimentSelections are calculation inputs only: each identifies an existing experimentId and exact existing columnIds from activeExperimentCatalog.",
  "Never put a desired new workbook field or a guessed columnId in experimentSelections.",
  "Requests naming a workbook, worksheet, Excel column, or new source field must use sourceSelections plus source_field fieldTargets.",
  "Use sourceFieldCandidates as deterministic hints for explicit column letters and field-name matches, and verify them against confirmedRegions.",
  "fieldTargets contains only scalar scientific output fields, never the experiment identity column.",
  "A direct workbook field target uses kind source_field plus its confirmed regionUnderstandingRevisionId and Excel column. Leave fieldKey, displayName, role, valueType, and unit empty because the backend derives them from accepted region semantics.",
  "A calculated output uses kind derived_field and must declare fieldKey, displayName, role, valueType, unit, and a readable description before review.",
  "Allowed field roles are identifier, condition, outcome, series_summary, and other. Allowed value types are number, string, date, and boolean.",
  ...
  properties: {
    ...
    fieldTargets: {
      ...
      items: {
        type: "object",
        properties: {
          kind: { type: "string", enum: ["source_field", "derived_field"] },
          regionUnderstandingRevisionId: { type: "string" },
          column: { type: "string" },
          ...
          role: {
            type: "string",
            enum: ["", "identifier", "condition", "outcome", "series_summary", "other"],
          },
          valueType: {
            type: "string",
...
const EXPERIMENT_BROWSER_PROGRAM_SYSTEM = [
  ...
  "Return exactly {pythonProgram:{runtime,entrypoint,source}}.",
  "The entrypoint must be def analyze(inputs, labrat).",
  "inputs is a dictionary. inputs['tables'], inputs['experiments'], inputs['fieldCatalog'], and inputs['targetFields'] are always lists, never dictionaries keyed by ID.",
  ...
  "Generated Python cannot call either inspection tool or methods with those names on labrat; it must read inputs['tables'], inputs['experiments'], inputs['fieldCatalog'], and inputs['targetFields'] directly.",
  ...
  "The accepted targetFields list is authoritative for every scalar output definition. Python must not invent or repeat field metadata.",
  "Every upsertFields item must provide exactly targetFieldId, value, formattedValue, confidence, warnings, and sources, plus missingReason only when value is None.",
  "targetFieldId must be copied exactly from inputs['targetFields']; never output fieldKey, displayName, role, valueType, unit, or columnId.",
  "The backend merges the accepted target definition and determines whether the field is new or replaces an existing column.",
  ...
  "browserView may provide name, visibleColumnIds, filters, and sort. Use targetFields[].columnId for target scalar columns. visibleColumnIds contains scalar field columnIds only; never add series IDs because series appear in experiment details and comparisons rather than Browser table columns. Newly published views are opened directly but never replace the user's default view.",
