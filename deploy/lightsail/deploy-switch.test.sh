...
ORIGINAL_MODE="$(file_mode "$ENV_FILE")"
...
[[ "$(file_mode "$ENV_FILE")" == "$ORIGINAL_MODE" ]] || fail_test "file mode changed."
...
if [[ "$SYMLINK_ASSERTION_SUPPORTED" == "true" ]]; then
  [[ "$(resolved_link_target "$CURRENT_LINK")" == "$(cd "$OLD_RELEASE" && pwd -P)" ]] \
...
if [[ "$POSIX_METADATA_ASSERTIONS" == "true" ]]; then
  [[ "$(file_mode "$ENV_FILE")" == "$ORIGINAL_MODE" ]] || fail_test "rollback changed file mode."
fi
