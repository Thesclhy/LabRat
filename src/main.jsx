...
import { useWorkbookBatchActions, workbookBatchSummaryText } from "./hooks/useWorkbookBatch.js";
import { WorkbookBatchCard, templateMatchDetail } from "./components/WorkbookBatchCard.jsx";
...
export function AgentPanel({
  ...
  const workbookBatch = useWorkbookBatchActions({
    ...
    onBatchUpdate: (batchId, updater, { phase } = {}) => updateWorkbookBatchMessage(batchId, (message) => {
      const nextBatch = updater(message.workbookBatch || { batchId, items: [] });
      return phase === "upload"
        ? { text: workbookBatchSummaryText(nextBatch.items), workbookBatch: nextBatch }
        : { workbookBatch: nextBatch };
    }),
  });
  ...
  const uploadWorkbookAttachments = async (files, next) => {
    ...
    if (spreadsheetAttachments.length > 1) {
      ...
      workbookBatch.rememberFiles(batchId, spreadsheetAttachments);
        ...
        <div className="msg-body">
          ...
          {m.workbookBatch && (
            <WorkbookBatchCard
              ...
              canRetry={workbookBatch.hasFiles(m.workbookBatch.batchId)}
              retrying={workbookBatch.retryingBatchId === m.workbookBatch.batchId}
              templates={asArray(projectState?.regionExtractionTemplates)}
              matching={workbookBatch.matchingBatchId === m.workbookBatch.batchId}
              applying={workbookBatch.applyingBatchId === m.workbookBatch.batchId}
              confirming={workbookBatch.confirmingBatchId === m.workbookBatch.batchId}
              experiments={workbookBatch.experiments}
