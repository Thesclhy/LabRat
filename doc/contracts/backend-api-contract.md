...
- Durable charts have two evidence-backed origins: source-extract ChartSpecs contain immutable source snapshots, while analysis-result ChartSpecs contain exact accepted-analysis hashes, input snapshot refs, validated trace arrays, and source-record lineage.
- Analysis-result ChartSpec creation occurs only through exact-result-hash acceptance after execution and validation. Project/list payloads contain bounded trace metadata; `GET /api/chart-specs/:chartSpecId` returns the complete immutable artifact.
